...
public class WebviewFragment extends TitlebarFragment implements OnKeyDownAble {
    ...
    protected SimpleInjectJsMobileAgent onConfigMobildAgentObject() {
        ...
        //发送umeng统计
        simpleInjectJsMobileAgent.setUmengEventHandler(new UmengEventHandler() {
            @Override
            public void onUmengEvent(String eventId, String eventName) {
                UmengAgent.onEvent(getActivity(), eventName);
            }
        });
        //处理调用js方法后的回调
    ...
    public static class MyBackkeydownHelper implements CallWindowFunCallbacActionHandler {
        ...
        public MyBackkeydownHelper(WebviewFragment webviewFragmentWebView) {
            this.webviewFragmentWeakReference = new WeakReference<WebviewFragment>(webviewFragmentWebView);
        }
        ...
        public void onCallWindowFunCallback(JSONArray result, String funName, String requestCode, int responseCode) {
            ...
            if ("onBackKeyDown".equals(funName)) {//如果js没处理  back键事件 则app处理，如果用户做了处理，则app什么也不做
                ...
                final WebviewFragment webviewFragment = webviewFragmentWeakReference == null ? null : webviewFragmentWeakReference.get();
                //切换到UI线程
        ...
        public boolean notifyBackKeyDownToJs() {
            WebviewFragment webviewFragment = webviewFragmentWeakReference == null ? null : webviewFragmentWeakReference.get();
